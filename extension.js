...
var MprisLabel = GObject.registerClass(
	...
	class MprisLabel extends PanelMenu.Button {
		...
		_onPaddingChanged() {
			...
			if (SHOW_ICON) {
				if (ICON_PLACE == 'right') RIGHT_PADDING = Math.max(0, RIGHT_PADDING - 5);
				else LEFT_PADDING = Math.max(0, LEFT_PADDING - 5);
			}

			this.box.set_style('padding-left: ' + LEFT_PADDING + 'px;' + 'padding-right: ' + RIGHT_PADDING + 'px; ');
		}
		...
		_onScroll(event) {
			...
			if (SCROLL_ACTION == 'none') return Clutter.EVENT_STOP;
			...
			if (event.get_scroll_direction() == Clutter.ScrollDirection.SMOOTH) {
				...
				if (!delta == 0) this._changeVolume(delta);
		...
		_changeVolume(delta) {
			...
			if (CONTROL_SCHEME == 'application' && this.player) {
				if (this.stream.length == 0) this._getStream();
			...
			stream.forEach((stream) => {
				//if multiple stream, use the lowest as base reference
				if (stream.volume < volume) volume = stream.volume;
			});
		...
		_buildMenu() {
			...
			this.players.list.forEach((player) => {
				...
				settingsMenuItem.connect('activate', () => {
					if (AUTO_SWITCH_TO_MOST_RECENT) this.settings.set_boolean('auto-switch-to-most-recent', false);
		...
		_setIcon() {
			...
			if (!ICON_PLACE || !this.player || this.label.get_text() == '' || this.label.get_text() == PLACEHOLDER) return;
			...
			if (USE_ALBUM) {
				...
				if (!blacklist.includes(this.player.identity.toLowerCase())) this.icon = this.player.getArtUrlIcon(size);
			}
			...
			if ((this.icon != null) | undefined) {
				...
				else if (ICON_PLACE == 'left') this.box.insert_child_at_index(this.icon, 0);
			}
		}
