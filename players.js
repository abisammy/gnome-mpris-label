...
var Players = class Players {
	...
	pick() {
		...
		if (this.list.includes(this.selected) && !AUTO_SWITCH_TO_MOST_RECENT) return this.selected;
	...
	updateFilterList() {
		...
		if (!SOURCES_BLACKLIST || !SOURCES_WHITELIST) this.list = this.unfilteredList;
		...
		if (!USE_WHITELIST && SOURCES_BLACKLIST)
			this.list = this.unfilteredList.filter(
				(element) => !blacklist.includes(element.identity.toLowerCase().replaceAll(' ', '')),
			);
	}
	updateActiveList() {
		let actives = [];
...
class Player {
	...
	_onEntryProxyReady() {
		...
		let matchedEntries = [];
		if (!((this.identity == null) | undefined)) matchedEntries = Gio.DesktopAppInfo.search(this.identity);

		if (matchedEntries.length === 0 && !((this.desktopEntry == null) | undefined))
		...
		if (matchedEntries.length > 0) this.desktopApp = this._matchRunningApps(matchedEntries);
	...
	getIcon(desktopApp) {
		...
		if (Config.PACKAGE_VERSION.startsWith('3.'))
			if (ICON_PLACE == 'right') icon_left_padding = 3;
			else if (ICON_PLACE == 'left') icon_right_padding = 3;
		...
		if ((desktopApp == null) | undefined) return icon;
