...
var buildLabel = function buildLabel(players) {
	...
	if (REMOVE_TEXT_WHEN_PAUSED) {
		if (removeTextWhenPaused(players.selected)) return placeholder;
	}

	let metadata = players.selected.metadata;

	if (metadata == null) return placeholder;
	...
	if (labelstring.length === 0) return placeholder;

	return labelstring;
};

function removeTextWhenPaused(player) {
	if (player.playbackStatus == 'Playing') return false;
...
function parseMetadataField(data) {
	...
	if (data.includes('xesam:') || data.includes('mpris:')) return '';
	...
	if (data.length > MAX_STRING_LENGTH) {
		...
		if (lastIndex == -1) lastIndex = data.length;
