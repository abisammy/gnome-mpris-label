...
function buildPrefsWidget() {
	...
	if (shellVersion >= 40) {
		//workaround taken directly from gjs.guide
	...
	let showIconComboBox = addStringComboBox(
		panelPage,
		'show-icon',
		'Show source icon:',
		{off: '', left: 'left', right: 'right'},
		undefined,
	);
	...
	let extensionPlaceComboBox = addStringComboBox(
		panelPage,
		'extension-place',
		'Extension place:',
	...
	addSpinButton(
		...
		0,
		20,
		'Set widget location within with respect to other adjacent widgets',
	);
	...
	addSpinButton(
		panelPage,
		'reposition-delay',
		'Panel reposition at startup (delay in seconds):',
		0,
		300,
		"Increase this value if extension index isn't respected at startup",
	);
	...
	addSwitch(
		...
		'This option can be annoying without the use of filter lists',
	);
	...
	addEntry(
		labelPage,
		'button-placeholder',
		'Button placeholder (can be left empty):',
		'The button placeholder is a hint for the user\nAppears when the label is empty and another available source is active',
	);
	...
	let leftClickComboBox = addStringComboBox(
		controlsPage,
		'left-click-action',
		'Left click action:',
		buttonActions,
	...
	let middleClickComboBox = addStringComboBox(
		controlsPage,
		'middle-click-action',
		'Middle click action:',
		buttonActions,
	...
	let rightClickComboBox = addStringComboBox(
		controlsPage,
		'right-click-action',
		'Right click action:',
		buttonActions,
	...
	let scrollComboBox = addStringComboBox(
		controlsPage,
		'scroll-action',
		'Scroll up/down action:',
		{'volume controls': 'volume-controls', none: 'none'},
		undefined,
	);
	let thumbForwardComboBox = addStringComboBox(
		controlsPage,
		'thumb-forward-action',
		'Thumb-tip button action:',
		buttonActions,
		undefined,
	);
	let thumbBackwardComboBox = addStringComboBox(
		controlsPage,
		'thumb-backward-action',
		'Inner-thumb button action:',
		buttonActions,
		undefined,
	);

	addSubcategoryLabel(controlsPage, 'Behaviour');
	let VolumeControlComboBox = addStringComboBox(
		controlsPage,
		'volume-control-scheme',
		'Volume control scheme:',
		{application: 'application', global: 'global'},
		undefined,
	);
...
function addLabel(widget, labelstring, labeltooltip) {
	...
	if (labeltooltip) thisLabel.set_tooltip_text(labeltooltip);
...
function buildLabel(labelstring) {
	//don't confuse with label.js buildLabel
